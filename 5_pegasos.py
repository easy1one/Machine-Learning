...
def objective_function(X, y, w, lamb):
    ...
    for i in range(len(X)):
        sum += max(0,1-y[i]*np.dot(w.T,X[i]))
    obj_value = lamb*(np.dot(w.T,w))/2.0 + sum / N

    return obj_value.flatten()[0]
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1): #, max_iterations + 1
        A_t = np.floor(np.random.rand(k) * N).astype(int)  # index of the current mini-batch
        sum_A = np.zeros((D,1), dtype=int)
        for a_idx in A_t:
            wByX = np.dot(w.T,Xtrain[a_idx])
            if(ytrain[a_idx] * wByX < 1.0):
                row_inX = np.reshape(Xtrain[a_idx],(D,1))
                sum_A  = sum_A + (ytrain[a_idx]*np.array(row_inX))
        eta = 1/(lamb*iter)
        w = (1-(eta*lamb))*w + (eta/k)*sum_A
        w = min(1.0, 1.0/(np.sqrt(lamb))/np.linalg.norm(w)) * w
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    """
    numOfCorrect = 0
    numOfSamples = len(ytest)
    test_out = np.dot(np.array(Xtest),w)
    for i in range(len(test_out)):
        test_out[i] = 1 if(test_out[i] >= t) else -1
    for i in range(numOfSamples):
        if test_out[i] == ytest[i]:
            numOfCorrect += 1
    return numOfCorrect / numOfSamples
